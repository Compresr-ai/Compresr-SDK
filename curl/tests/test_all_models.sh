#!/bin/bash
# Test all supported compression models

set -e

# Load test configuration
# shellcheck source=./config.sh
. "$(dirname "$0")/config.sh"
FAILED=0
PASSED=0

echo "========================================="
echo "Testing all supported compression models"
echo "========================================="
echo ""

# Test context
CONTEXT="Machine learning is a subset of artificial intelligence that enables systems to learn from data. Deep learning uses neural networks with multiple layers. Natural language processing helps computers understand human language. Computer vision allows machines to interpret images."
QUERY="What is machine learning?"

# Test 1: latte_v2 (token-level)
echo "1. Testing latte_v2 (token-level)..."
RESPONSE=$(curl -s -X POST "$BASE_URL/api/compress/question-specific/" \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $COMPRESR_API_KEY" \
  -d "{
    \"context\": \"$CONTEXT\",
    \"query\": \"$QUERY\",
    \"compression_model_name\": \"latte_v2\",
    \"coarse\": false,
    \"source\": \"sdk:curl\"
  }")

if echo "$RESPONSE" | jq -e '.data.compressed_context' > /dev/null 2>&1; then
    echo "   ✓ latte_v2 (token-level) passed"
    ((++PASSED))
else
    echo "   ✗ latte_v2 (token-level) failed"
    echo "   Response: $RESPONSE"
    ((++FAILED))
fi
echo ""

# Test 2: latte_v2 (paragraph-level / coarse)
echo "2. Testing latte_v2 (paragraph-level / coarse)..."
RESPONSE=$(curl -s -X POST "$BASE_URL/api/compress/question-specific/" \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $COMPRESR_API_KEY" \
  -d "{
    \"context\": \"$CONTEXT\",
    \"query\": \"$QUERY\",
    \"compression_model_name\": \"latte_v2\",
    \"coarse\": true,
    \"source\": \"sdk:curl\"
  }")

if echo "$RESPONSE" | jq -e '.data.compressed_context' > /dev/null 2>&1; then
    echo "   ✓ latte_v2 (paragraph-level) passed"
    ((++PASSED))
else
    echo "   ✗ latte_v2 (paragraph-level) failed"
    echo "   Response: $RESPONSE"
    ((++FAILED))
fi
echo ""

echo "Results: $PASSED passed, $FAILED failed"
echo "========================================="

if [ $FAILED -gt 0 ]; then
    exit 1
fi

echo "✓ All model tests passed!"
exit 0
